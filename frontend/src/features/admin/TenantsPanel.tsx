...
export function TenantsPanel({ tenants, areas, reload }: {
  ...
  const [form, setForm] = useState({ name: '', slug: '', parentTenantId: 'tenant_tmgs', theme: 'satourn', active: true, areaIds: [] as string[] });
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState('');

  function openEditor(tenant: AdminTenant | 'new') {
    setMessage('');
    setEditing(tenant);
    setForm(tenant === 'new' ? {
      name: '', slug: '', parentTenantId: 'tenant_tmgs', theme: 'satourn', active: true, areaIds: []
    } : {
      name: tenant.name, slug: tenant.slug, parentTenantId: tenant.parent_tenant_id || '',
      theme: tenant.theme, active: tenant.active, areaIds: tenant.area_ids || []
    });
  }
  ...
  return (
    <section className="panel-card admin-section-card">
      <header className="admin-section-header">
        ...
        <button className="tool-primary-button" type="button" onClick={() => openEditor('new')}>Nutzergruppe anlegen</button>
      </header>
