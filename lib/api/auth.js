...
export async function approvedLoginEmail(email) {
  ...
  const [profile] = await sql.query(`
    SELECT profile.role, profile.email, tenant.slug AS tenant_slug
  ...
  return Boolean(
    profile
    && isValidApplicationRole(profile.role)
    && isEmailAllowedForTenant(profile.email, profile.tenant_slug)
  );
}
...
export function publicIdentity(identity) {
  return {
    ...
    allowedAreaIds: identity.allowed_area_ids,
