...
import { buildDestinationOneCategoriesUrl, buildDestinationOneImportUrl, flattenDestinationOneCategories, parseDestinationOneResponse } from './destination-one.js';
...
export async function syncEt4ToNeon(options = {}) {
  ...
  for (const recordType of selectedTypes) {
    ...
    try {
      const categoryResponse = await fetch(buildDestinationOneCategoriesUrl({ baseUrl, experience, type: recordType, apiKey }), { headers: { Accept: 'application/json' }, signal: AbortSignal.timeout(requestTimeoutMs) });
      if (!categoryResponse.ok) throw new Error(`ET4 categories returned HTTP ${categoryResponse.status} for ${recordType}.`);
      const categoryCatalog = flattenDestinationOneCategories(await categoryResponse.json());
      await upsertCategoryCatalog(sql, { experience, taxonomy: recordType, categories: categoryCatalog });
