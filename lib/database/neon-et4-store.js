...
export async function reconcileOpenDataMembership(sql, {
  syncToken,
  inventoryExperience,
  openDataExperience,
  recordType = 'All'
  ...
  const [counts] = await sql`
    SELECT
      COUNT(record.global_id)::integer AS published_count,
      COUNT(*) FILTER (WHERE record.global_id IS NULL)::integer AS unmatched_count,
      (
        SELECT COUNT(*)::integer
        FROM et4_records
        WHERE experience = ${inventoryExperience}
          AND is_active = TRUE
          AND is_open_data_published = TRUE
      ) AS previous_published_count
    FROM et4_sync_seen AS seen
    LEFT JOIN et4_records AS record
      ON record.experience = ${inventoryExperience}
     AND record.global_id = seen.global_id
     AND record.is_active = TRUE
    WHERE seen.sync_token = ${syncToken}
      AND seen.experience = ${openDataExperience}
      AND seen.record_type = ${recordType}
  `;

  const seenCount = counts.published_count + counts.unmatched_count;
  if (seenCount > 0 && counts.published_count / seenCount < 0.9) {
    throw new Error(`Open-data membership matched only ${counts.published_count}/${seenCount} inventory records.`);
  }
  if (counts.previous_published_count > 0 && counts.published_count < counts.previous_published_count * 0.8) {
    throw new Error(
      `Open-data membership dropped unexpectedly from ${counts.previous_published_count} to ${counts.published_count}.`
    );
  }

  await sql`
    UPDATE et4_records AS record
    SET is_open_data_published = EXISTS (
      SELECT 1
      FROM et4_sync_seen AS seen
      WHERE seen.sync_token = ${syncToken}
        AND seen.experience = ${openDataExperience}
    ...
    WHERE record.experience = ${inventoryExperience}
      AND record.is_open_data_published IS DISTINCT FROM EXISTS (
        SELECT 1
        FROM et4_sync_seen AS seen
        WHERE seen.sync_token = ${syncToken}
          AND seen.experience = ${openDataExperience}
  ...
  return counts || { published_count: 0, unmatched_count: 0, previous_published_count: 0 };
}
