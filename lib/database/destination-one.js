...
export function buildDestinationOneCategoriesUrl({ baseUrl, experience, type, apiKey }) {
  const url = new URL('../categories/', baseUrl);
  url.searchParams.set('experience', experience);
  url.searchParams.set('type', type);
  url.searchParams.set('mode', 'json');
  url.searchParams.set('mkt', 'de-DE');
  if (apiKey) url.searchParams.set('licensekey', apiKey);
  return url.toString();
}

export function flattenDestinationOneCategories(payload, parentName = null) {
  if (!Array.isArray(payload)) return [];
  return payload.flatMap((entry) => {
    const name = String(entry?.name || '').trim();
    if (!name) return [];
    return [{ name, parentName, externalId: null }, ...flattenDestinationOneCategories(entry.children, name)];
  });
}
