...
function authBaseUrl() {
  const value = String(process.env.NEON_AUTH_BASE_URL || '').trim().replace(/\/$/, '');
...
export async function runNeonAuthRequest(request, path, bodyOverride) {
  ...
  const hasBody = Boolean(body && request.method !== 'GET' && request.method !== 'HEAD');
...
export async function getNeonAuthSession(request) {
  ...
  const authResponse = await runNeonAuthRequest({ ...request, method: 'GET' }, 'get-session');
