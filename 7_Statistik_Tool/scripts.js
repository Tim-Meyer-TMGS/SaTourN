document.addEventListener('DOMContentLoaded', () => {
    ...
    const buildUrl = (type, query = '', isOpenData = false) => {
        const base       = 'https://meta.et4.de/rest.ashx/search/?experience=statistik_sachsen';
        const licenseKey = '&licensekey=0846fglkc42n3710gba&';
        let qParam = '';

        if (query) {
            // query beginnt typischerweise mit "&q=..."
            if (isOpenData) {
                // Open-Data: license-Filter ergänzen
                // Entferne führendes "&" und füge "q=" hinzu, falls nötig
                const rawQuery = query.startsWith('&q=') ? query.slice(3) : query;
                qParam = `&q=${rawQuery}+AND+attribute_license%3A(CC0+OR+CC-BY+OR+CC-BY-SA)`;
            } else {
                qParam = query; // z.B. "&q=area%3A%22...%22+AND+category%3A%22...%22"
            }
        } else if (isOpenData) {
            // Fall: keine sonstigen Filter, aber Open-Data-Grundabfrage
            qParam = '&q=attribute_license%3A(CC0+OR+CC-BY+OR+CC-BY-SA)';
        }

        return `${base}${licenseKey}type=${type}${qParam}&template=ET2014A.xml`;
    };
