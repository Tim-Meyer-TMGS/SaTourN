...
const [summary] = await sql.query(`
  SELECT
    ...
    (SELECT COUNT(*)::integer FROM "account" WHERE "providerId" = 'credential' AND password IS NOT NULL AND LENGTH(password) > 40) AS hashed_password_accounts,
...
if (summary.hashed_password_accounts < summary.users) throw new Error('At least one user has no stored password hash.');
