# Datenqualitaets-Monitor - Projektstand Abschnitte 1 bis 11
...
Bis Abschnitt 11 ist die Uebersichtsebene inklusive klickbarer Fehlerlisten, Datentyp-Uebersicht, Qualitaetskriterien-Matrix, kontextsensitivem CSV-Export, vorbereitetem KI-Chatbot und Datensatz-Detailansicht umgesetzt.
...
- Erledigt mit Abschnitt 11: `buildAiDetailContext()` gibt den aktuell geoeffneten Detaildatensatz reduziert an den KI-Kontext weiter.

## Abschnitt 11 - Detailansicht pro Datensatz

Umgesetzt in `Statistik/index.html`, `Statistik/style.css` und `Statistik/scripts.js`:

- eigenes Seitenpanel `item-detail-panel` fuer Datensatzdetails.
- Detailpanel kann geschlossen werden.
- Fehlerlisten-Zeilen sind klickbar und per Tastatur mit Enter/Leertaste oeffnbar.
- Aggregierte Ergebnislisten-Zeilen oeffnen ein passendes Beispiel aus den im Browser vorhandenen Qualitaetsdaten, sofern ein Sample-Datensatz zur Zeile existiert.
- Links in der Fehlerliste bleiben normale externe Links und oeffnen nicht versehentlich das Detailpanel.
- Ausgewaehlter Detaildatensatz wird in `dashboardState.activeDetailItem` gehalten.
- `buildAiDetailContext()` gibt nun den reduzierten Detaildatensatz fuer den Chatbot-Kontext zurueck.
...
- Qualitaetsstatus
- Letzte Aktualisierung
- URL / Link zum Datensatz
- fehlende Kriterien
- erfuellte Kriterien
- manuell zu pruefende Kriterien
- empfohlene Massnahmen
- verwendete bzw. vermutete Datenfelder pro Kriterium
- Rohdaten optional einklappbar

Erklaerung fehlender Kriterien:

- Jedes fehlende Kriterium wird als eigene Karte angezeigt.
- Pro Kriterium werden angezeigt:
  - Label
  - Prioritaet
  - automatische Pruefbarkeit
  - Statushinweis
  - vermutete Datenfelder
  - Empfehlung
  - vorhandene Feldwerte, soweit im normalisierten Datensatz oder Rohdatensatz auffindbar

Manuelle Kriterien:

- Manuell zu pruefende Kriterien werden separat angezeigt.
- Sie enthalten Label, Prioritaet, Hinweis zur manuellen Pruefung und Empfehlung.
- Diese Kriterien werden nicht als automatisch fehlend bewertet.

Rohdaten:

- Rohdaten sind nicht standardmaessig offen.
- Sie werden in einem einklappbaren `details`-Block angezeigt.
- Dadurch bleibt die Detailansicht fachlich lesbar und grosse JSON-Strukturen uebernehmen nicht die Ansicht.

Einschraenkung:

- Die Ergebnisliste ist aggregiert. Ein Klick auf eine Ergebniszeile kann deshalb nur einen Beispiel-Datensatz aus der aktuell im Browser vorhandenen Stichprobe zeigen, nicht zwingend jeden Datensatz dieser Aggregatzeile.
- Wenn fuer eine Aggregatzeile kein Sample-Datensatz vorhanden ist, wird die Zeile nicht als Detailzeile markiert.

TODOs aus Abschnitt 11:

- Detailpanel spaeter visuell und inhaltlich mit finalen echten destination.one-Feldnamen schaerfen.
...
- Direkte Detailansicht fuer Ergebnislisten-Aggregate fachlich klaeren: Beispiel-Datensatz, Liste der Sample-Datensaetze oder separate Detailauswahl.
- Optional Copy-/Export-Aktion fuer einzelne Datensatzdetails ergaenzen.
- Optional Linkanzeige im Detailpanel als klickbaren Button statt Text darstellen.
...
14. Klick auf eine Fehlerlisten-Zeile oeffnet die Datensatz-Detailansicht fuer genau diesen Datensatz.
15. Klick auf eine aggregierte Ergebnislisten-Zeile oeffnet, falls vorhanden, einen Beispiel-Datensatz aus dem Browser-Sample.
16. Ein geoeffneter Detaildatensatz wird als reduzierter Kontext in `buildAiDetailContext()` an den KI-Kontext angebunden.
...
- `docs/Codex/Datenqualitaetsmonitor_Datenbasis_und_Limits.md`
- `docs/Codex/Datenqualitaetsmonitor_Python_Entscheidung.md`
- `docs/Codex/Datenqualitaetsmonitor_Projektstand_Abschnitte_1_bis_11.md`
- `docs/Codex/Zielbild.md`
- `docs/Codex/SaTourN_Codex_Prompt_Datenqualitaetsmonitor.md`
...
- Filter, Empty States und Robustheit in Abschnitt 12 schaerfen.
