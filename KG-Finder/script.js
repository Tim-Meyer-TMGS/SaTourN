...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function setStatus(msg, isError=false){
    ...
    statusEl.innerHTML = isError ? `<span style="color:${'rgba(255,113,113,.95)'}">${escapeHtml(msg)}</span>` : msg;
  }

  function setLoading(on){
    if (on) setStatus(`<span class="kg-spinner"></span> Lädt…`);
  }
  ...
  async function search(query){
    ...
    try{
      ...
      if (!things.length){
        setStatus('Keine Treffer. Begriff ändern.');
        return;
      }

      const detailed = await Promise.all(
        things.map(t => fetchThingById(t['@id']).catch(() => null))
      );

      for (const entry of detailed.filter(Boolean)){
        await renderItem(entry, { tpl, results });
      }

      setStatus(`${things.length} Treffer.`);
      prevBtn.disabled = (page === 1);
      nextBtn.disabled = (things.length < PAGE_SIZE);
      countEl.textContent = `Seite ${page}`;
    }catch(err){
      console.error(err);
      setStatus(err?.message || 'Unerwarteter Fehler', true);
    }
  }
});
...
/* ==========================
   Image resolution (Onlim ImageObject)
========================== */
...
async function loadImageObjectContentUrl(onlimEntityUrl){
  try{
    ...
    const toStr = (v) =>
      Array.isArray(v) ? (typeof v[0]==='string' ? v[0] : (v[0]?.['@value'] || v[0]?.['value'] || '')) :
